...
func TestFindStatic(t *testing.T) {
	ints := []int{1, 2, 3, 4}
	if v, ok := iters.Find(ints, func(x int) bool { return x == 3 }); !ok || v != 3 {
		t.Errorf("funcs.Find() = (%d, %v), want (3, true)", v, ok)
	}

	if v, ok := iters.Find(ints, func(x int) bool { return x == 5 }); ok {
...
func TestFindIndexStatic(t *testing.T) {
	ints := []int{1, 2, 3, 4}
	index := iters.FindIndex(ints, func(x int) bool { return x == 3 })
	if index != 2 {
		t.Errorf("funcs.FindIndex() = %d, want %d", index, 2)
	}

	index = iters.FindIndex(ints, func(x int) bool { return x == 5 })
	if index != -1 {
...
func TestFindFirstStatic(t *testing.T) {
	...
	if v, ok := iters.FindFirst(strs, func(s string) bool { return strings.HasPrefix(s, "ban") }); !ok ||
		v != "banana" {
		t.Errorf("funcs.FindFirst() = (%s, %v), want (banana, true)", v, ok)
	}

	if v, ok := iters.FindFirst(strs, func(s string) bool { return s == "durian" }); ok {
...
func TestFindLastStatic(t *testing.T) {
	...
	if v, ok := iters.FindLast(strs, func(s string) bool { return strings.HasPrefix(s, "ban") }); !ok ||
		v != "banana" {
		t.Errorf("funcs.FindLast() = (%s, %v), want (banana, true)", v, ok)
	}

	if v, ok := iters.FindLast(strs, func(s string) bool { return s == "durian" }); ok {
...
func TestSomeStatic(t *testing.T) {
	ints := []int{1, 2, 3, 4}
	if !iters.Some(ints, func(x int) bool { return x > 3 }) {
		t.Errorf("funcs.Some() returned false, want true")
	}
	if iters.Some(ints, func(x int) bool { return x > 10 }) {
...
func TestNoneStatic(t *testing.T) {
	ints := []int{1, 3, 5, 7}
	if !iters.None(ints, func(x int) bool { return x%2 == 0 }) {
		t.Errorf("funcs.None() returned false, want true")
	}
	mixed := []int{1, 3, 4, 7}
	if iters.None(mixed, func(x int) bool { return x%2 == 0 }) {
...
func TestFindOnStringsStatic(t *testing.T) {
	...
	if v, ok := iters.Find(strs, func(s string) bool { return len(s) == 5 }); !ok || v != "alpha" {
		t.Errorf("funcs.Find() on strings = (%s, %v), want (alpha, true)", v, ok)
	}

	index := iters.FindIndex(strs, func(s string) bool { return s == "delta" })
	if index != 3 {
		t.Errorf("funcs.FindIndex() on strings = %d, want %d", index, 3)
	}

	if v, ok := iters.FindLast(strs, func(s string) bool { return s[0] == 'b' }); !ok ||
		v != "beta" {
...
func TestEmptySliceQueriesStatic(t *testing.T) {
	var ints []int
	if _, ok := iters.Find(ints, func(x int) bool { return true }); ok {
		t.Errorf("funcs.Find() on empty slice returned true, want false")
	}

	if idx := iters.FindIndex(ints, func(x int) bool { return true }); idx != -1 {
		t.Errorf("funcs.FindIndex() on empty slice = %d, want -1", idx)
	}

	if _, ok := iters.FindLast(ints, func(x int) bool { return true }); ok {
		t.Errorf("funcs.FindLast() on empty slice returned true, want false")
	}

	if iters.Some(ints, func(x int) bool { return true }) {
		t.Errorf("funcs.Some() on empty slice returned true, want false")
	}

	if !iters.All(ints, func(x int) bool { return false }) {
		t.Errorf("funcs.All() on empty slice returned false, want true (vacuously)")
	}

	if !iters.None(ints, func(x int) bool { return true }) {
...
func TestFind(t *testing.T) {
	ints := iters.LiftSlice([]int{1, 2, 3, 4})
...
func TestFindFirst(t *testing.T) {
	strs := iters.LiftSlice([]string{"apple", "banana", "cherry", "bana"})
...
func TestFindLast(t *testing.T) {
	strs := iters.LiftSlice([]string{"apple", "bana", "cherry", "banana"})
...
func TestAll(t *testing.T) {
	ints := iters.LiftSlice([]int{2, 4, 6, 8})
	if !ints.All(func(x int) bool { return x%2 == 0 }) {
		t.Errorf("funcs.All() returned false, want true")
	}
	mixed := iters.LiftSlice([]int{2, 4, 5, 8})
...
func TestNone(t *testing.T) {
	ints := iters.LiftSlice([]int{1, 3, 5, 7})
	if !ints.None(func(x int) bool { return x%2 == 0 }) {
		t.Errorf("funcs.None() returned false, want true")
	}
	mixed := iters.LiftSlice([]int{1, 3, 4, 7})
...
func TestCount(t *testing.T) {
	ints := iters.LiftSlice([]int{1, 2, 3, 4, 5, 6})
...
func TestFindOnStrings(t *testing.T) {
	strs := iters.LiftSlice([]string{"alpha", "beta", "gamma", "delta"})
